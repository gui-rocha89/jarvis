...
describe('Cross-Channel Identity (Sprint 4)', () => {
  ...
  it('detecta nome genérico (evita falso positivo cross-channel)', async () => {
    const { _internal } = await import('../src/contacts.mjs');
    assert.equal(_internal._isGenericName('João'), true, 'João é nome genérico');
    assert.equal(_internal._isGenericName('Maria'), true);
    assert.equal(_internal._isGenericName(''), true, 'vazio é genérico');
    assert.equal(_internal._isGenericName('ab'), true, 'menos de 3 chars é genérico');
    assert.equal(_internal._isGenericName('Kayque Torrubia'), false, 'nome incomum não é genérico');
